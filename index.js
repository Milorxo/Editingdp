...
function openFullscreenContentModal(type, date) {
    ...
    if (type === 'tasks') {
        ...
        if (!hasContent) domElements.fullscreenModalArea.innerHTML = '<p>No tasks were completed on this day.</p>';
...
function showCategoryContextMenu(categoryId, tabButton) {
    ...
    domElements.ctxDeleteCategoryButton.style.display = currentCategories.find(c => c.id === categoryId)?.deletable !== false ? 'block' : 'none';
    ...
    domElements.categoryTabContextMenu.dataset.categoryId = categoryId;
}
...
function openEnterFolderNameModal(type) {
    if (!domElements.enterFolderNameModal || !tempFolderCreationData) return;
    tempFolderCreationData.type = type;
    closeChooseFolderTypeModal();
    if (domElements.enterFolderNameTitle) domElements.enterFolderNameTitle.textContent = `Name Your New ${type.charAt(0).toUpperCase() + type.slice(1)} Folder`;
...
function createNewFolder() {
    ...
    const newFolderId = createUniqueId(`folder-${categoryId}`);
    const newFolder = {
        id: newFolderId,
        name: folderName,
        type: type,
...
/**
 * Adjusts the layout of progress bar containers and main content padding
 * based on the current screen width.
 * On larger screens (PC), progress bars are moved to a dedicated sidebar.
 * On smaller screens (tablet/mobile), they are moved to a designated mobile location.
 * This function is a key part of the application's responsive design strategy.
 */
function updateLayoutBasedOnScreenSize() {
    if (!domElements.appProgressSidebar || !domElements.mobileProgressLocation || !domElements.todayProgressContainer || !domElements.currentWeekProgressContainer || !domElements.mainContentWrapper) {
        // console.warn("One or more DOM elements for layout update are missing.");
        return;
    }
    const pcBreakpoint = 1025; // Matches CSS

    if (window.innerWidth >= pcBreakpoint) {
        // PC View: Move progress to sidebar
        if (!domElements.appProgressSidebar.contains(domElements.todayProgressContainer)) {
            domElements.appProgressSidebar.appendChild(domElements.todayProgressContainer);
        }
        if (!domElements.appProgressSidebar.contains(domElements.currentWeekProgressContainer)) {
        ...
        // Adjust padding for main content if menu toggle is visible AND sidebar is present
        // This logic might need refinement based on actual menu toggle state
        domElements.mainContentWrapper.style.paddingLeft = '45px'; 
    } else {
        // Mobile/Tablet View: Move progress to mobile location
        if (!domElements.mobileProgressLocation.contains(domElements.todayProgressContainer)) {
            domElements.mobileProgressLocation.appendChild(domElements.todayProgressContainer);
        }
        if (!domElements.mobileProgressLocation.contains(domElements.currentWeekProgressContainer)) {
        ...
        // Adjust padding for main content based on menu toggle visibility
        domElements.mainContentWrapper.style.paddingLeft = domElements.menuToggleButton && getComputedStyle(domElements.menuToggleButton).display !== 'none' ? '65px' : '30px'; // Example values
    }
}


function toggleAppMenu() {
    if (!domElements.appMenuPanel || !domElements.menuToggleButton) return;
    const isHidden = domElements.appMenuPanel.classList.toggle('hidden');
    domElements.appMenuPanel.setAttribute('aria-hidden', isHidden.toString());
    domElements.menuToggleButton.setAttribute('aria-expanded', (!isHidden).toString());
    if (!isHidden) {
        domElements.appMenuPanel.querySelector('button')?.focus();
    }
}
...
function showMenuSelectedView(viewId, title) {
    if (!domElements.menuSelectedContentView || !domElements.menuSelectedContentTitle || !domElements.menuSelectedContentArea) return;
    
    domElements.menuSelectedContentTitle.textContent = title;
    
    // Hide all content areas first
    domElements.menuSelectedContentArea.querySelectorAll('.menu-view-content').forEach(content => content.classList.add('hidden'));

    let targetViewElement = null;
    if (viewId === 'progress') {
        targetViewElement = domElements.progressView;
        if (targetViewElement) {
             // Clone today & weekly progress and put them into #progress-view
            targetViewElement.innerHTML = ''; // Clear previous
            if (domElements.todayProgressContainer && domElements.currentWeekProgressContainer) {
                const todayClone = domElements.todayProgressContainer.cloneNode(true);
                const weeklyClone = domElements.currentWeekProgressContainer.cloneNode(true);
                todayClone.removeAttribute('id'); // Avoid duplicate IDs
                weeklyClone.removeAttribute('id');

                // Ensure clones have appropriate styling if needed, or that classes carry over.
                // For simplicity, direct append. May need adjustment if styles are ID-specific.
                targetViewElement.appendChild(todayClone);
                targetViewElement.appendChild(weeklyClone);
            } else {
                targetViewElement.textContent = "Progress details are not available at the moment.";
            }
        }
    } else if (viewId === 'clock') {
        targetViewElement = domElements.liveClockView;
        if (targetViewElement) startLiveClock();
    } else if (viewId === 'dashboard') {
        targetViewElement = domElements.dashboardColumnView;
        if (targetViewElement) updateDashboardSummaries(); // Ensure it's up-to-date
    }

    if (targetViewElement) {
        targetViewElement.classList.remove('hidden');
    } else {
        domElements.menuSelectedContentArea.innerHTML = `<p>Content for ${title} not found.</p>`; 
    }
    
    domElements.menuSelectedContentView.classList.remove('hidden');
    domElements.appViewWrapper.classList.add('hidden'); // Hide main app view
    closeAppMenu();
    domElements.closeMenuSelectedViewButton.focus();
}

function closeMenuSelectedView() {
    if (!domElements.menuSelectedContentView || !domElements.appViewWrapper) return;
    domElements.menuSelectedContentView.classList.add('hidden');
    domElements.appViewWrapper.classList.remove('hidden'); // Show main app view
    stopLiveClock();
...
function startLiveClock() {
    if (!domElements.hourHand || !domElements.minuteHand || !domElements.secondHand) return;
    ...
    function setClock() {
        ...
        const secondsDegrees = ((seconds / 60) * 360) + 90; // +90 because default is 12 o'clock
        const minutesDegrees = ((minutes / 60) * 360) + ((seconds/60)*6) + 90;
        const hoursDegrees = ((hours / 12) * 360) + ((minutes/60)*30) + 90;

        domElements.secondHand.style.transform = `translateX(-50%) rotate(${secondsDegrees}deg)`;
        domElements.minuteHand.style.transform = `translateX(-50%) rotate(${minutesDegrees}deg)`;
        domElements.hourHand.style.transform = `translateX(-50%) rotate(${hoursDegrees}deg)`;
    }
    setClock(); // Initial set
    liveClockInterval = setInterval(setClock, 1000);
}
...
function initializeDOMReferences() {
  const ids = [
    'menu-toggle-button', 'app-menu-panel', 'menu-item-progress', 'menu-item-clock', 'menu-item-dashboard',
    'app-view-wrapper', 'menu-selected-content-view', 'menu-selected-content-header', 'menu-selected-content-title',
    'close-menu-selected-view-button', 'menu-selected-content-area', 'dashboard-column', 'live-clock-view',
    'hour-hand', 'minute-hand', 'second-hand', 'progress-view',
    'app-progress-sidebar', 'mobile-progress-location', 'main-content-wrapper',
    'tabs', 'tab-content', 'add-category-button', 'category-section-template',
    ...
    'dashboard-summaries', 'today-progress-fill', 'today-points-stat',
    'current-week-progress-fill', 'current-week-points-stat',
    'today-progress-container', 'current-week-progress-container',
    'calendar-month-year-button', 'calendar-month-year', 'calendar-grid',
    'calendar-prev-month', 'calendar-next-month',
    'month-year-picker-modal', 'month-year-picker-content', 'month-year-picker-close-button',
    'picker-months-grid', 'picker-years-list',
    'daily-note-input', 'save-note-button',
    'history-modal', 'history-modal-close-button', 'history-modal-date',
    'history-modal-points-value', 'history-modal-points-total',
    'history-percentage-progress-fill', 'history-tasks-list', 'expand-tasks-button',
    'historical-reflection-wrapper', 'expand-reflection-button', 'history-user-note-display',
    'history-user-note-edit', 'historical-note-controls', 'save-historical-note-button',
    'clear-historical-note-button', 'historical-note-status',
    'task-edit-controls-template', 'delete-confirmation-modal', 'delete-confirmation-title',
    'delete-confirmation-message', 'delete-confirmation-close-button', 'confirm-delete-button',
    'cancel-delete-button', 'fullscreen-content-modal', 'fullscreen-modal-title',
    'fullscreen-modal-area', 'fullscreen-modal-close-button',
    'choose-folder-type-modal', 'choose-folder-type-close-button', 'select-task-folder-button', 'select-note-folder-button',
    'enter-folder-name-modal', 'enter-folder-name-close-button', 'enter-folder-name-title', 'folder-name-input',
    'create-folder-button', 'cancel-create-folder-button', 'image-upload-input',
  ];
  ids.forEach(id => {
    const camelCaseId = id.replace(/-([a-z])/g, g => g[1].toUpperCase());
    domElements[camelCaseId] = document.getElementById(id);
  });
  domElements.dashboardColumnView = domElements.dashboardColumn; // alias
}

function addEventListeners() {
    // Menu System
    domElements.menuToggleButton?.addEventListener('click', toggleAppMenu);
    domElements.menuItemProgress?.addEventListener('click', () => showMenuSelectedView('progress', 'Progress Overview'));
    domElements.menuItemClock?.addEventListener('click', () => showMenuSelectedView('clock', 'Live Clock'));
    domElements.menuItemDashboard?.addEventListener('click', () => showMenuSelectedView('dashboard', 'Activity Dashboard'));
    domElements.closeMenuSelectedViewButton?.addEventListener('click', closeMenuSelectedView);

    // Global click listener for closing menus/modals
    document.addEventListener('click', (event) => {
        if (domElements.appMenuPanel && !domElements.appMenuPanel.classList.contains('hidden') && !domElements.appMenuPanel.contains(event.target) && !domElements.menuToggleButton.contains(event.target)) {
            closeAppMenu();
        }
        if (domElements.categoryTabContextMenu && !domElements.categoryTabContextMenu.classList.contains('hidden') && !domElements.categoryTabContextMenu.contains(event.target) && (!currentContextMenuTargetTab || !currentContextMenuTargetTab.contains(event.target))) {
            hideCategoryContextMenu();
        }
        if (domElements.folderOptionsContextMenu && !domElements.folderOptionsContextMenu.classList.contains('hidden') && !domElements.folderOptionsContextMenu.contains(event.target) && (!currentContextMenuTargetFolderBox || !currentContextMenuTargetFolderBox.contains(event.target))) {
            hideFolderContextMenu();
        }
    });
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            if (domElements.menuSelectedContentView && !domElements.menuSelectedContentView.classList.contains('hidden')) {
                closeMenuSelectedView();
            } else if (domElements.appMenuPanel && !domElements.appMenuPanel.classList.contains('hidden')) {
                closeAppMenu();
                domElements.menuToggleButton?.focus();
                ...
                closeHistoryModal();
            } else if (domElements.monthYearPickerModal && !domElements.monthYearPickerModal.classList.contains('hidden')) {
                closeMonthYearPicker();
            } else if (domElements.deleteConfirmationModal && !domElements.deleteConfirmationModal.classList.contains('hidden')) {
                hideDeleteConfirmation();
            } else if (domElements.enterFolderNameModal && !domElements.enterFolderNameModal.classList.contains('hidden')) {
                closeEnterFolderNameModal();
            } else if (domElements.chooseFolderTypeModal && !domElements.chooseFolderTypeModal.classList.contains('hidden')) {
                closeChooseFolderTypeModal();
            ...
            } else if (domElements.folderOptionsContextMenu && !domElements.folderOptionsContextMenu.classList.contains('hidden')) {
                hideFolderContextMenu();
                currentContextMenuTargetFolderBox?.closest('.folder-item-container')?.focus();
            }
        }
    });
    ...
    domElements.ctxRenameCategoryButton?.addEventListener('click', () => {
        ...
        if (newName && newName.trim() !== "" && newName.trim() !== category.name) {
            ...
            saveUserCategories(currentCategories);
            renderTabs(); 
            document.querySelector(`#category-section-${categoryId} .category-title-text`)?.textContent = newName.trim();
        }
        hideCategoryContextMenu();
    });
    ...
    domElements.ctxDeleteCategoryButton?.addEventListener('click', () => {
        ...
        const category = currentCategories.find(c => c.id === categoryId);
        if (!category) return;
        showDeleteConfirmation('category', categoryId, `Are you sure you want to delete the category "${category.name}" and all its content? This action cannot be undone.`, category.name);
        hideCategoryContextMenu();
    });
    ...
    domElements.ctxDeleteFolderButton?.addEventListener('click', () => {
        ...
        const folder = findFolderById(folderId);
        if (!folder) return;
        showDeleteConfirmation('folder', folderId, `Are you sure you want to delete the folder "${folder.name}" and all its content? This action cannot be undone.`, folder.name, categoryId);
        hideFolderContextMenu();
    });

    // Daily Note
    domElements.saveNoteButton?.addEventListener('click', saveDailyNote);
    domElements.dailyNoteInput?.addEventListener('input', () => {
        if (domElements.saveNoteButton) domElements.saveNoteButton.textContent = 'Save Note';
    });
    ...
    domElements.deleteConfirmationModal?.addEventListener('click', (e) => { if (e.target === domElements.deleteConfirmationModal) hideDeleteConfirmation(); });

    // Folder Creation Modals
    domElements.chooseFolderTypeCloseButton?.addEventListener('click', closeChooseFolderTypeModal);
    domElements.chooseFolderTypeModal?.addEventListener('click', (e) => { if (e.target === domElements.chooseFolderTypeModal) closeChooseFolderTypeModal(); });
    domElements.selectTaskFolderButton?.addEventListener('click', () => openEnterFolderNameModal('task'));
    domElements.selectNoteFolderButton?.addEventListener('click', () => openEnterFolderNameModal('note'));
    domElements.enterFolderNameCloseButton?.addEventListener('click', closeEnterFolderNameModal);
    domElements.enterFolderNameModal?.addEventListener('click', (e) => { if (e.target === domElements.enterFolderNameModal) closeEnterFolderNameModal(); });
    ...
    domElements.folderNameInput?.addEventListener('keypress', (e) => { if(e.key === 'Enter') createNewFolder(); });

    // Task List Drag & Drop (event delegation on tabContentsContainer)
    domElements.tabContentsContainer?.addEventListener('dragover', (e) => {
        const taskList = e.target.closest('.task-list');
        if (draggedTaskElement && taskList && currentFolderEditModes[draggedTaskElement.dataset.folderId]) {
            e.preventDefault();
            ...
            if (afterElement == null) {
                if (taskList.lastElementChild && taskList.lastElementChild !== draggedTaskElement) {
                    taskList.lastElementChild.classList.add('drag-over-indicator-task-bottom');
                }
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  updateLayoutBasedOnScreenSize(); // Initial layout check
